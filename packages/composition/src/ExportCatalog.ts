...
export class ExportCatalog {
   ...
   */
  private exportedTypes: { [contractType:string] : { [contractName:string] : any} };

  /**
   * Hashset of shared instance of exported class indexed by contract type/contract name
   */
  private sharedInstances: { [contractType:string] : { [contractName:string] : any} };
  ...
  getExport(contractType: string, contractName: string, ...constructorArgs:any) {
    ...
    const metadata = exportedClass.metadata.find((m:ExportMetadata) => m.contractType === contractType && m.contractName === contractName);
    ...
    else
    {
      if(Array.isArray(constructorArgs) && constructorArgs.length > 0){
        ...
        metadata.constructorInjectedArgs.forEach((injectedArg:any) => {
          let arg:any = null;
