...
import {NotInCatalogError} from "./NotInCatalogError";
...
export class InstancesFactory {
  ...
  constructor(directoryCatalogRoot?:string) {
      if(!directoryCatalogRoot) {
        if(!require || !require.main)
            throw new Error('A directory catalog must be provided as the current file can\'t be find');
  ...
  getInstanceFromCatalogs(contractType:string, contractName:string, ...constructorArgs:any):any {
    ...
    throw new NotInCatalogError(contractType, contractName);
  }
}
